...
export default function EditIngredient() {
  ...
  const loadItem = async () => {
    try {
      ...
      Alert.alert("Error", "Failed to load ingredient");
      router.back();
    } finally {
      setLoading(false);
    }
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const itemData: any = {
        ...
        available: available,
        status:
          available && parseFloat(quantity) > 0 ? "in_stock" : "out_of_stock",
      };
      ...
      const response = await updateInventoryItem(itemId, itemData);
  ...
  if (loading) {
    return (
      <View style={styles.loadingContainer}>
        ...
        <Text style={styles.loadingText}>Loading ingredient...</Text>
      </View>
    );
  }
